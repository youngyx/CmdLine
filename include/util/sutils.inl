...
{
	return (wchar_t*)::wcschr(str, val&0xFFFF);
}
...
SP_INLINE
int normalize_path(char path[MAX_PATH*2], const char path_split /* = SLASH_CHR */)
{
	int len = 0;
	if ( 0 == path )
		return 0;
...
int cat_audio_file(const char * file, const void * data, int bytes, bool fail_if_exist /* = false */)
{
	...
	if ( audio_head_len == 0 && file_size >= 44 )
	{
		...
		fseek(fp, 0, SEEK_SET);
		fread(file_head, 1, sizeof file_head, fp);
...
int read_text_file(const char * file, std::wstring & text)
{
	...
	if ( fs )
	{
		...
		while ( !fs.eof() )
		{
			...
			fs.get(cnt, sizeof cnt, -1&0xFFFF);
			text += cnt;
		}
		return 0;
	}
